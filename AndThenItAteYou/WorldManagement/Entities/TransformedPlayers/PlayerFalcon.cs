...
{
    public class PlayerFalcon : TransformedPlayer
    {
        ...
        public PlayerFalcon(Vector2 location, WorldBase world, Player transformedFrom) : base(location, world, transformedFrom)
        {
            this.gravityMultiplier = .7f;
            this.jumpForce = .1f;
            this.walkSpeed = .15f;
            this.frictionMultiplier = .7f;
            this.width = 50;
            this.height = 50;
        ...
        public override void prePhysicsUpdate(GameTime time)
        {
            ...
            if (flapping)
            {
                ...
                if (frameSwitcher <= 0)
                {
                    if (currentFrame + 1 == Game1.texture_falcon_flap.Length)
                    {
                        flapping = false;
                    }else
                    {
                        currentFrame = (currentFrame + 1) % Game1.texture_falcon_flap.Length;
                        frameSwitcher = flapFrameSwitchPoint;
                        selectedFrame = Game1.texture_falcon_flap[currentFrame];
                    }
            ...
            else
            {
                if (frameSwitcher <= 0)
                {
                    currentFrame = (currentFrame + 1) % Game1.texture_falcon.Length;
                    frameSwitcher = frameSwitchPoint;
                    selectedFrame = Game1.texture_falcon[currentFrame];
                }
            }
