...
{
    public class Item_Spear_Fanged : Item
    {

        public Item_Spear_Fanged(int uses) : base(uses)
        {
            if(texture == null)
            {
                texture = Game1.texture_item_spear_fanged;
            }
        }

        public override Item clone(int uses)
        {
            return new Item_Spear_Fanged(uses);
        }
        ...
        public override int use(PlayerBase user, WorldBase world, Vector2 location, GameTime time, BinaryInputManager inputManager)
        {
            ...
            if (user is Player)
            {
                ...
                {
                    EntitySpearFanged spear = new EntitySpearFanged(user.location + new Vector2(0, -15), world, user);
                    spear.velocity += Vector2.Normalize(location - user.location) * 17;
                    world.addEntity(spear);
                    used++;
                    player.state.decorate(spear);
                    player.state.submitStateAction(STATE_ACTIONS.THROW);
                    SoundManager.getSound("spear-throw").playWithVariance(0, .4f, 0, SoundType.MONSTER);
                }
            }
            else
            {
